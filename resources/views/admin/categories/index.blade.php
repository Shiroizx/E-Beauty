...
@section('title', 'Manajemen Kategori — Admin Skinbae.ID')
...
<div class="space-y-5">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-xl font-bold tracking-tight text-gray-900">Manajemen Kategori</h2>
            <p class="mt-0.5 text-sm text-gray-400">Atur kategori produk skincare</p>
        </div>
        <a href="{{ route('admin.categories.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-brand-500 to-brand-600 px-4 py-2.5 text-sm font-semibold text-white shadow-md shadow-brand-500/20 transition hover:shadow-lg hover:shadow-brand-500/25">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-stone-100 text-left">
                        <th class="py-3 pl-5 pr-3 text-[11px] font-semibold uppercase tracking-wider text-gray-400">Kategori</th>
                        <th class="px-3 py-3 text-[11px] font-semibold uppercase tracking-wider text-gray-400">Parent</th>
                        ...
                        <tr class="transition hover:bg-stone-50/60">
                            <td class="py-3 pl-5 pr-3">
                                <div class="flex items-center gap-3">
                                    @if($category->icon)
                                        <img src="{{ Storage::url($category->icon) }}" alt="" class="h-8 w-8 rounded-lg object-cover ring-1 ring-stone-100">
                                    @else
                                        <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-stone-100 text-stone-400">
                            ...
                            <td class="px-3 py-3">
                                @if($category->parent)
                                    <span class="inline-flex items-center rounded-lg bg-stone-50 px-2 py-0.5 text-xs font-medium text-gray-600 ring-1 ring-inset ring-stone-200/60">{{ $category->parent->name }}</span>
                                @else
                                    <span class="text-xs text-gray-300">—</span>
                                @endif
                            </td>
                            <td class="px-3 py-3">
                                <span class="inline-flex items-center rounded-full bg-stone-100 px-2 py-0.5 text-xs font-semibold text-gray-600">{{ $category->products_count }}</span>
                            </td>
                            <td class="px-3 py-3">
                            ...
                            <td class="py-3 pl-3 pr-5">
                                <div class="flex items-center justify-end gap-1.5">
                                    <a href="{{ route('admin.categories.edit', $category->id) }}" class="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 transition hover:bg-blue-50 hover:text-blue-500">
                        ...
                        <tr>
                            <td colspan="5" class="py-16 text-center">
                                <div class="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-stone-100">
                                    <i class="fas fa-folder-open text-stone-300 text-lg"></i>
                                </div>
