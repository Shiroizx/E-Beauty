...
@section('title', 'Edit Brand — Admin Skinbae.ID')
...
<div class="mx-auto max-w-lg space-y-5">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold tracking-tight text-gray-900">Edit Brand</h2>
            <p class="mt-0.5 text-sm text-gray-400">{{ $brand->name }}</p>
        </div>
    ...
    <div class="rounded-2xl bg-white p-5 shadow-card">
        <form action="{{ route('admin.brands.update', $brand->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                @if($brand->logo)
                    <div class="mb-2">
                        <img src="{{ Storage::url($brand->logo) }}" alt="Preview" class="h-12 rounded-lg ring-1 ring-stone-200">
                    </div>
                @endif
            ...
            <div>
                ...
                <textarea name="description" rows="4" class="w-full rounded-xl border border-stone-200 px-3.5 py-2.5 text-sm text-gray-700 transition focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-100">{{ old('description', $brand->description) }}</textarea>
            </div>
            ...
            <button type="submit" class="w-full rounded-xl bg-gradient-to-r from-brand-500 to-brand-600 py-2.5 text-sm font-bold text-white shadow-md shadow-brand-500/20 transition hover:shadow-lg hover:shadow-brand-500/25">
                <i class="fas fa-save mr-2 text-xs"></i> Update Brand
            </button>
        </form>
    </div>
</div>
