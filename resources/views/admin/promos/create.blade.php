...
@section('title', 'Buat Promo — Admin Skinbae.ID')
...
<div class="mx-auto max-w-2xl space-y-5">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold tracking-tight text-gray-900">Buat Promo Baru</h2>
            <p class="mt-0.5 text-sm text-gray-400">Atur detail kode promo dan diskon</p>
        </div>
        ...
        <form action="{{ route('admin.promos.store') }}" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="Contoh: Diskon Lebaran"
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code') }}" required placeholder="LEBARAN2026" style="text-transform:uppercase"
                           class="w-full rounded-xl border border-stone-200 px-3.5 py-2.5 text-sm font-mono text-gray-700 placeholder-gray-300 transition focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-100">
                </div>
            </div>

            <div>
                <label class="mb-1.5 block text-xs font-semibold text-gray-600">Deskripsi</label>
                <textarea name="description" rows="3" class="w-full rounded-xl border border-stone-200 px-3.5 py-2.5 text-sm text-gray-700 transition focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-100">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="number" name="usage_limit" value="{{ old('usage_limit') }}" min="0"
                           class="w-full rounded-xl border border-stone-200 px-3.5 py-2.5 text-sm text-gray-700 transition focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-100">
                    <p class="mt-1 text-[11px] text-gray-400">Kosongkan jika tidak terbatas</p>
                </div>
                <div>
                    <label class="mb-1.5 block text-xs font-semibold text-gray-600">Minimum Pembelian</label>
                    <input type="number" name="min_purchase" value="{{ old('min_purchase') }}" min="0" step="0.01"
                           class="w-full rounded-xl border border-stone-200 px-3.5 py-2.5 text-sm text-gray-700 transition focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-100">
                </div>
            </div>
            ...
            <div>
                ...
                <p class="mt-1 text-[11px] text-gray-400">Pilih produk jika promo hanya berlaku untuk produk tertentu</p>
            </div>
            ...
            <button type="submit" class="w-full rounded-xl bg-gradient-to-r from-brand-500 to-brand-600 py-2.5 text-sm font-bold text-white shadow-md shadow-brand-500/20 transition hover:shadow-lg hover:shadow-brand-500/25">
                <i class="fas fa-save mr-2 text-xs"></i> Simpan Promo
            </button>
        </form>
    </div>
</div>
